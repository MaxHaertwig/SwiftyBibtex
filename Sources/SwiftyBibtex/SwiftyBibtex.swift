...
public struct SwiftyBibtex {
    public static func parse(_ input: String) throws -> [Publication] {
        ...
        return processParsedPublications(publicationListener.publications)
    }

    public static func parseComments(_ input: String) throws -> [String] {
        let commentListener = BibtexCommentListener()
        let bibtexParser = parser(for: input)
        try ParseTreeWalker().walk(commentListener, try bibtexParser.root())
        return commentListener.comments
    }
